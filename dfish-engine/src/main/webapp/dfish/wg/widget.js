...
// `viewlayout` view的占据空间的widget，可见元素都隶属于此
...
Checkbox = define.widget( 'checkbox', {
	...
	Prototype: {
		...
		html: function() {
			var w = this.innerWidth(), s = this.prop_cls(), t = this.attr( 'tip' ), y = '';
			if ( w ) {
				y += 'width:' + w + 'px;';
			} else {
			...
			return '<' + this.tagName + ' id=' + this.id + ' class="' + s + (this.x.nobr ? ' f-fix' : '') + '"' + (t && typeof t !== _OBJ ? 'title="' + $.strQuot( (t === T ? this.x.text : t) || '' ) + '"' : '') +
	...
	Prototype: {
		...
		html: function() {
			return '<' + this.tagName + ' id=' + this.id + ' class="' + this.prop_cls() + (this.x.checkstate == 2 ? ' z-half' : '') + '"' + (this.x.id ? ' w-id="' + this.x.id + '"' : '') + '><input type=checkbox id=' + this.id + 't name="' + this.x.name + '" value="' + (this.x.value || '') + '" class=_t' +
				(this.x.checkstate == 1 ? ' checked' : '') + (this.isDisabled() ? ' disabled' : '') + (this.x.partialsubmit ? ' w-partialsubmit="1"' : '') + _html_on.call( this ) + '>' + this.html_text() + '</' + this.tagName + '>';
		}
	}
} ),
/* `switch` */
...
Select = define.widget( 'select', {
	...
	Prototype: {
		...
		html_nodes: function() {
			...
			var z = this.x.size, t = (t === T ? (o[ k ] && o[ k ].text) : t) || '';
...
Leaf = define.widget( 'leaf', {
	...
	Prototype: {
		...
		html_self: function( a ) {
			...
			return '<dl class="' + this.className + (x.cls ? ' ' + x.cls : '') + (c ? ' z-line' : '') + (!p ? ' z-root' : '') + (this.isFirst() ? ' z-first' : '') + (this.isLast() ? ' z-last' : '') + (this.isDisabled() ? ' z-ds' : '') + (x.src || a ? ' z-folder' : '') + (this.isFolder() && x.open ? ' z-open' : '') + (this.isEllipsis() && !this.textNode ? ' f-omit' : ' f-nobr') +
				...
				(this.box ? this.box.html() : '') + this.html_icon() + '<cite class=w-leaf-t id=' + this.id + 't>' + t + '</cite></dt>' + this.html_after() + '</dl>';
		},
		html: function() {
