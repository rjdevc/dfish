...
//	AbstractBox<?> box;
	...
	/**
	 * 排序-默认 {@link GridColumnSort#STATUS_DEFAULT}
	 */
	@Deprecated
	public static final String SORT_DEFAULT = GridColumnSort.STATUS_DEFAULT;
	/**
	 * 排序-正序 {@link GridColumnSort#STATUS_ASC}
	 */
	@Deprecated
	public static final String SORT_ASC = GridColumnSort.STATUS_ASC;
	/**
	 * 排序-倒序 {@link GridColumnSort#STATUS_DESC}
	 */
	@Deprecated
	public static final String SORT_DESC = GridColumnSort.STATUS_DESC;
...
//		return new GridColumn( dataColumnIndex, field, null, width).setBox(new Radio(boxName, null, null, null,null).addValidate(new Validate().setRequired(required)).setField(new BoxField().setValue(field).setChecked(checkedField))).setAlign(GridColumn.ALIGN_CENTER);
	 ...
	 * @return 本身，这样可以继续设置其他属性
	 */
	public GridColumn setSort(GridColumnSort sort) {
		this.sort = sort;
		return this;
	}
...
//		} else
	...
	public String getSortsrc() {
		if (sort != null && sort instanceof GridColumnSort) {
			return ((GridColumnSort) sort).getSrc();
		}
		return null;
	}
	...
	public GridColumn setSortsrc(String sortsrc) {
		this.sort = new GridColumnSort().setSrc(sortsrc);
		return this;
	}
