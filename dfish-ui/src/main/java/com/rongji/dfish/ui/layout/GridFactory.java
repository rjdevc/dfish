...
/**
 * GridFactory 用于快速生成一个Grid
 * GridPanel 在DFish2.x的版本是一个很实用的工具。
 * 他为 表格模型封装类。
 * <p>为了延续DFish2.x 易用的特点点，保留GridPanel类。这个类不是DFish3.x系类的原型类，它仅仅提供是一个看起来是表格布局的控件。
 ...
 * <p>所以，5.0 独立出来，自行调用build 方法。获取Grid。grid除了id和cls属性可以设置外，其他都是build以后设置。
...
public class GridFactory {
    ...
    protected static abstract class AbstractGridFactory<T extends AbstractGridFactory<T>>{
        ...
        private static Object getProperty(Object item, String prop) {
            ...
            try {
                ...
                LogUtil.error(GridFactory.class, null, e);
                return null;
            }
        }
    ...
    public static class DefaultGridFactory extends AbstractGridFactory<DefaultGridFactory>{
        private Collection<?> gridData;
        public DefaultGridFactory(String id) {
            super(id);
        }
        /**
         * 设置表格数据
         ...
         * @return 本身，这样可以继续设置其他属性
         */
        public DefaultGridFactory setGridData(Collection<?> gridData) {
        ...
        @Override
        public Grid build() {
            Grid prototype=new Grid(super.getId());
     ...
     * 数据格式等。这些将不输出。
     */
    public static class Column extends Grid.Column{
