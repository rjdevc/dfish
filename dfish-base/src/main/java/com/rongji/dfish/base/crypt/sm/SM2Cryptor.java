...
public class SM2Cryptor extends StringCryptor{
	/** 素数p */
...
//		printHexString(h.toByteArray());
		if (publicKey != null && h!=null ) {
			ECPoint result = publicKey.multiply(h);
			if (!result.isInfinity()) {
				System.out.println("pass");
			} else {
				System.err.println("计算椭圆曲线点 S = [h]Pb失败");
				return null;
			}
		}
...
//		System.out.println("密文长度: "+encryptResult.length);
...
//			System.err.println("generate key failed");
	...
	public SM2Cryptor(String encoding, int presentStyle, Object arg) {
	...
	protected byte[] encrypt(byte[] src) throws Exception {
		SM2KeyPair pair = generateKeyPair();
		if(pair==null){
			throw new Exception(" generate SM2KeyPair failed !!! ");
		}
		return encrypt(src,pair.getPublicKey());
	}
	...
	protected byte[] decrypt(byte[] src) throws Exception {
		...
		return decrypt(src, generateKeyPair().getPrivateKey());
	}
