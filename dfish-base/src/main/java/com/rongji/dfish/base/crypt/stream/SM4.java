...
public class SM4 {
    ...
    public static void sm4_one_round(long[] sk, byte[] input,int inputOff, byte[] output,int outputOff)
    {
        int i = 0;
        long[] ulbuf = new long[36];
        ulbuf[0] = GET_ULONG_BE(input, inputOff);
        ulbuf[1] = GET_ULONG_BE(input, inputOff+4);
        ulbuf[2] = GET_ULONG_BE(input, inputOff+8);
        ulbuf[3] = GET_ULONG_BE(input, inputOff+12);
        while (i < 32)
        {
            ulbuf[(i + 4)] = sm4F(ulbuf[i], ulbuf[(i + 1)], ulbuf[(i + 2)], ulbuf[(i + 3)], sk[i]);
            i++;
        }
        PUT_ULONG_BE(ulbuf[35], output, outputOff);
        PUT_ULONG_BE(ulbuf[34], output, outputOff+4);
        PUT_ULONG_BE(ulbuf[33], output, outputOff+8);
        PUT_ULONG_BE(ulbuf[32], output, outputOff+12);
    }
    ...
    public static void sm4_setkey_enc(long[] SK, byte[] key){
        sm4_setkey(SK,key);
    }

    public static void sm4_setkey_dec(long[] SK, byte[] key){
        sm4_setkey(SK,key);
        for (int i = 0; i < 16; i++){
            SWAP(SK, i);
        }
