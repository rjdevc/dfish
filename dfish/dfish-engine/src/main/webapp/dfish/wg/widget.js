...
Scroll = define.widget( 'scroll', {
	Const: function( x ) {
		x.scroll && (this.className += ' w-scroll');
	...
	Prototype: {
		...
		checkScroll: function() {
			! mb && _reset_resize_sensor.call( this ) && _show_scroll.call( this );
		},
	...
	Listener: {
		...
		body: {
			focus: {
				...
				method: function( e ) {
					e == N && this.$t().focus();
					if ( mb ) { // mobile选中表单会弹出键盘，表单可能会被键盘遮住看不到，所以要让表单滚动到可视范围
			...
			change: {
				...
				method: function( e ) {
					...
					if ( this.lastValue != v ) {
						...
						f && this.cmd( { type: 'tip', id: this.id + 'mxltip', text: Loc.form.reach_maxlength } );
					}
				}
			},
			resize: function() {
			...
			click: {
				...
				method: function() {
					if ( ! this.isDisabled() ) {
						...
						if ( p.x.format && p.x.format.indexOf( 'h' ) > 0 ) {
...
Calendar = define.widget( 'calendar/date', {
	...
	Helper: {
		...
		pop: function( a, b, c, d, e, f, g ) {
			var o = _widget( a ), h = b.indexOf( 'h' ),
				x = { type: 'calendar/' + ( b == 'yyyy' ? 'year' : b == 'yyyy-mm' ? 'month' : b == 'yyyy-ww' ? 'week' : 'date' ), format: b, target: o, callback: g,
	...
	Prototype: {
		...
		backfill: function() {
			this.x.callback.call( this.x.target, $.dateFormat( this.date, this.x.format ) );
		},
		inputMonth: function() {
		...
		popTime: function( a ) {
			if ( this._dlg_time ) {
				...
				a.innerHTML = Loc.calendar.picktime;
			} else {
				...
				f.indexOf( 'h' ) > 0 && list( 'h', 24 );
				f.indexOf( 'i' ) > 0 && list( 'i', 60 );
				f.indexOf( 's' ) > 0 && list( 's', 60 );
				...
				a.innerHTML = Loc.calendar.backdate;
			}
		},
		...
		html_nodes: function() {
			...
			if ( (n = 7 - ( e.length % 7 )) > 1 )
				e.push( '<td colspan=' + n + '>&nbsp;' );
			s += e.join( '' ) + '</tbody></table></div>';
			if ( this.x.format && this.x.format.indexOf( 'h' ) > 0 ) {
				s += '<div class=w-calendar-time><div class=_time onclick=' + evw + '.popTime(this)>' + Loc.calendar.picktime + '</div><div class=_ok onclick=' + evw + '.chooseTime(this)>' + Loc.confirm + '</div></div>';
			}
			return s;
		}
	}
...
_Date = define.widget( 'date', {
	...
	Listener: {
		body: {
			click: {
				prop: T,
				method: function() {
					! mb && this.isNormal() && this.popCalendar();
				}
			},
			input: mb && {
				prop: T,
	...
	Prototype: {
		...
		html_nodes: function() {
			var v = this.x.value || '';
			return mb ? '<input type=' + (_date_formtype[ this.x.format ] || 'date') + this.form_prop( v && v.replace( ' ', 'T' ) ) + '><label id="' + this.id + 'a" for="' + this.id + 't" class="f-boxbtn f-fix _a" style="width:' + this.innerWidth() + 'px;text-indent:' + _input_indent() + 'px">' + (this.x.value || '') + '</label>' : '<input type=text' + this.form_prop() + '><em class="f-boxbtn" onclick=' + eve + '></em>';
		}
	}
} ),
/* `range` */
...
Spinner = define.widget( 'spinner', {
	...
	Prototype: {
		...
		width_minus: function() { return (mb ? 78 : _boxbtn_width) + _input_indent() },
		step: function( a ) {
			if ( this.isNormal() ) {
		...
		html_nodes: function() {
			return mb ? '<cite class="f-boxbtn _l" onclick=' + evw + '.step(-1)><i class=f-vi></i>-</cite><input type=number' + this.form_prop() + '><cite class="f-boxbtn _r" onclick=' + evw + '.step(1)><i class=f-vi></i>+</cite>' :
	...
	Prototype: {
		...
		dragstart: function( a, b ) {
			...
			var x = b.clientX, c = this.x.validate, m = (c && c.maxvalue) || 100, n = (c && c.minvalue) || 0, f = _number( a.style.left ), 
				...
				d = t && this.cmd({ type: 'tip', text: this.formatStr( t, [ this.x.value ] ), snap: a, snaptype: 'tb,bt', closable: F }), v = self.$v().value;
			$.moveup( function( e ) {
		...
		drop: function() {
			...
			if ( l ) {
				for ( var i = 0, v = this.val(); i < l; i ++ ) {
					s.push( '<div class="w-imgbox-c f-inbl' + (d[ i ].value == v ? ' z-on' : '') + '" onclick=' + evw + '.choose(' + i + ')>' + this.html_img( d[ i ] ) + '</div>' );
				}
